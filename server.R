...
shinyServer(function(input, output) {
  ...
  tscanTable <- reactive({
    ...
    exp.f <- filter(exp.l2m, cell_type %in% c(input$cell_type.ts, input$cell_type.ts2)) %>% group_by(miRNA.ts, cell_type) %>%
      summarise(mean_expression = mean(expression))  %>% collect()
    # filter targetscan file
    ts <- collect(filter(tscan.full, Transcript_ID == rep_tscript & abs(`context++_score`) > input$context_threshold) %>% 
      select(miRNA, Site_Type, UTR_start, UTR_end, `context++_score`))
    # join the two
    te.j <- left_join(ts, exp.f, by = c("miRNA" = "miRNA.ts"))
    ...
    if(input$hd == TRUE & (input$cell_type.ts != input$cell_type.ts2)){
      ...
      te.j <- na.omit(te.j) %>% 
        ...
        melt(c(1:5,8), c(6,7), variable.name = "cell_type", 
             value.name = "mean_expression") %>%
        left_join(te.j, ., by = c("miRNA", "Site_Type", "UTR_start", "UTR_end", 
                                  "context++_score", "cell_type", "mean_expression"))
